...
const trans = {
    en: {
        ...
        music: {
            ...
            fetch_tracks: {
                name: 'Tracklist',
                loading: 'Fetching your top tracks',
                fail: 'You have not listened to this artist before :('
            },
            fetch_albums: {
                name: 'Tracklist',
                loading: 'Fetching your top albums',
                fail: 'You have not listened to an album by this artist before :('
            },
            track_list_view: {
                tracklist: 'Tracklist',
...
(function() {
    ...
    function album_missing_a_tracklist() {
        ...
        if (tracklist == null) {
            ...
            tracklist.innerHTML = (`
                <div class="section-controls tracklist-controls" id="tracklist-controls">
        ...
        let tracklist_scrobbles_page = document.createElement('div');
        tracklist_scrobbles_page.classList.add('tracklist-scrobbles-page', 'buffer-standard');
        tracklist_scrobbles_page.innerHTML = (`
            <div class="loading-data-container">
                <p class="loading-data-text">${trans[lang].music.fetch_plays.loading}</p>
            </div>
        `);
        tracklist.appendChild(tracklist_scrobbles_page);
        ...
        if (scrobble_link == undefined) {
            ...
            fetch(scrobble_link)
                ...
                .then(function(html) {
                    ...
                    /*let track_controls = document.getElementById('tracklist-controls');
                    track_controls.innerHTML = (`
                        <h3 class="text-18">${trans[lang].music.fetch_plays.name}</h3>
                        <div class="view-buttons track-view-buttons">
                            <button class="btn view-item track-list-view-item" id="toggle-track_list_view-0" data-toggle="track_list_view" data-toggle-value="0" disabled>
                                ${trans[lang].music.track_list_view.tracklist}
    ...
    function patch_artist_pages() {
        let header = document.querySelector('.header-new--artist');
        if (header == null)
            return;
        ...
        let personal_stats = document.querySelector('.personal-stats-item--scrobbles');
        let my_avi = document.querySelector('.auth-avatar-desktop').getAttribute('src');
        let scrobble_count = '0';
        let scrobble_link;
        if (personal_stats != null) {
    ...
    function patch_artist_top_tracks(top_tracks, inner_top_tracks, inner_top_tracks_btn, inner_top_tracks_status, my_avi, scrobble_count, scrobble_link) {
        ...
        track_view_mode.innerHTML = (`
            <button class="btn view-item track-list-view-item" id="toggle-track_list_view-0" data-toggle="track_list_view" data-toggle-value="0" onclick="_update_item('track_list_view', 0)">
                ${trans[lang].music.track_list_view.top_tracks}
            </button>
            <button class="btn view-item track-list-view-item" id="toggle-track_list_view-1" data-toggle="track_list_view" data-toggle-value="1" onclick="_update_item('track_list_view', 1)">
                <img class="view-item-avatar" src="${my_avi}" alt="${auth}">${trans[lang].music.track_list_view.listens.replace('{count}', scrobble_count)}
            </button>
        ...
        let top_tracks_scrobbles_page = document.createElement('div');
        top_tracks_scrobbles_page.classList.add('top-tracks-scrobbles-page', 'buffer-standard');
        top_tracks_scrobbles_page.innerHTML = (`
            <div class="loading-data-container">
                <p class="loading-data-text">${trans[lang].music.fetch_tracks.loading}</p>
            </div>
        `);
    ...
    function patch_artist_top_albums(top_albums, inner_top_albums, inner_top_albums_btn, inner_top_albums_status, my_avi, scrobble_count, scrobble_link) {
        ...
        let album_view_mode = document.querySelector('div');
        album_view_mode.classList.add('view-buttons', 'album-view-buttons');
        album_view_mode.setAttribute('id', 'album-controls');
        album_view_mode.innerHTML = (`
            <button class="btn view-item album-view-item" id="toggle-album_view-0" data-toggle="album_view" data-toggle-value="0" onclick="_update_item('album_view', 0)">
                ${trans[lang].music.album_view.top_albums}
            </button>
            <button class="btn view-item album-view-item" id="toggle-album_view-1" data-toggle="album_view" data-toggle-value="1" onclick="_update_item('album_view', 1)">
                <img class="view-item-avatar" src="${my_avi}" alt="${auth}">${trans[lang].music.album_view.listens.replace('{count}', scrobble_count)}
            </button>
        `);
        controls.appendChild(album_view_mode);

        refresh_all();

        let top_albums_scrobbles_page = document.createElement('div');
        top_albums_scrobbles_page.classList.add('top-albums-scrobbles-page', 'buffer-standard');
        top_albums_scrobbles_page.innerHTML = (`
            <div class="loading-data-container">
                <p class="loading-data-text">${trans[lang].music.fetch_albums.loading}</p>
            </div>
        `);
