    ...
    for (let setting in settings) {
        ...
        if (settings_store[setting]) {
            ...
            if (settings_store[setting].css) {
                document.body.style.setProperty(`--${settings_store[setting].css}`, `${settings[setting]}${settings_store[setting].suffix || ""}`);
            }
...
export function update_inbuilt_item(
    ...
    if (inbuilt_settings[item].type == "toggle") {
        ...
        element.querySelector(`#inbuilt-companion-checkbox-${item}`).checked =
            false;
        element
