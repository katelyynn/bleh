...
export function pixel({
	...
	render(
		...
		html`
			...
			<div class="setting-group">
				<div class="setting v" data-type="text">
					...
					<div class="input-container content-form">
			            <input type="text" class="input" ref=${(el) =>
				inputter = el} placeholder=${tl(
					trans.enter_a_profile,
				)} value=${page.requested.profile} onchange=${(e) => {
			...
			<button class="primary icon jumbo" data-type="pixel" onclick=${() =>
				pixel_prepare()}>
			    ${tl(trans.reset)}
			</button>
		`,
			...
			html`
				<div class="pixel-home">
					...
					<div class="pixel-guess center">
						<button class="primary jumbo continue" onclick=${() =>
							pixel_prepare()}>
				            ${tl(trans.begin)}
				        </button>
					</div>
	...
	function pixel_prepare() {
		...
		if (user_albums.length == 0) {
			fetch(
				`http://ws.audioscrobbler.com/2.0/?method=user.gettopalbums&user=${page.name}&api_key=${api_key}&format=json&limit=500`,
			)
				.then((res) => {
					if (!res.ok) {
						notify({
							id: 'api-error',
							type: 'error',
							title: tl(trans.value_failed_to_load).replace(
								'{v}',
								tl(trans.albums),
							),
							body: res.statusText,
	...
	function pixel_random() {
		pixel_guess(
			user_albums[Math.floor(Math.random() * user_albums.length)],
		);
	}
	...
	function pixel_guess({
			...
			html`
				...
				<div class="pixel-info">
					<div class="pixel-album-name">
						<h1 ref=${(el) => title_elem = el}>${jumble_string(
							name,
						)}</h1>
					</div>
				</div>
				<div class="pixel-actions" ref=${(el) => actions = el}>
					<button class="icon" data-type="add" onclick=${() =>
						pixel_hint()}>
				        ${tl(trans.add_hint)}
				    </button>
					...
					<button class="icon" data-type="minus" onclick=${() =>
						pixel_give_up()}>
				        ${tl(trans.give_up)}
				        ${keybind(['ESC'])}
				    </button>
				...
				<div class="pixel-guess">
				    ...
				    ${() => {
					const btn = html.node`
                        <button class="primary icon guess" data-type="send" ref=${(
						el,
					) => guess_btn = el} onclick=${() => guess_input.submit()}>
                            ${tl(trans.guess)}
                        </button>
                    `;
				...
				<div class="pixel-info">
					...
					<div class="hints" ref=${(el) =>
						hints_container = el}></div>
				</div>
			`,
		);
		...
		function render_canvas(override = false) {
			...
			log(
				`drawing canvas image with pixelation ${value} (${pixelation})`,
				'pixel',
			);
		...
		function pixel_end() {
			...
			render(
				actions,
				html`
					<button class="icon" data-type="pixel" onclick=${() =>
						pixel_random()}>
					    ${tl(trans.continue)}
					</button>
				`,
