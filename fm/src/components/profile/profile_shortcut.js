	...
	dialog({
		...
		body: html.node`
            <div class="big-modal-alert alert-danger">
                ${{
			html: tl(trans.profile_shortcut.notice).replace(
				'{u}',
				`<a class="mention" href="${root}user/${settings.profile_shortcut}" target="_blank">@${settings.profile_shortcut}</a>`,
			),
		}}
            </div>
            <div class="modal-footer">
                <button class="see-more cancel left-icon" onclick=${() =>
			dialog_rm({ id: 'profile_shortcut' })}>
                    ${tl(trans.back)}
                </button>
                <div class="fill"></div>
